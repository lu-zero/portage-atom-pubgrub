...
impl ConvertCtx<'_> {
            ...
            DepEntry::UseConditional {
                ...
                match state {
                    ...
                    UseFlagState::SolverDecided => {
                        ...
                        let virtual_pkg = PortagePackage::new(
                            portage_atom::Cpn::parse(&format!(
                                "virtual/USE_{}_{}",
    ...
    fn push_unslotted_or_choice(&mut self, cpn: &Cpn, version_set: PortageVersionSet) {
        ...
        match target_slots {
            ...
            Some(slots) => {
                ...
                let choice_pkg = PortagePackage::new(
                    portage_atom::Cpn::parse(&format!("virtual/slot_{id}")).unwrap(),
                    None,
                );
                let n = slots.len();
                let versions: Vec<(Version, Vec<(PortagePackage, PortageVersionSet)>)> = slots
                    ...
                    .enumerate()
                    .map(|(i, (_, slot_pkg))| {
                        let ver = Version::parse(&(n - i).to_string()).unwrap();
    ...
    fn convert_choice_group(&mut self, children: &[DepEntry], allow_none: bool) {
        let id = next_choice_id();
        let pkg = PortagePackage::new(
            portage_atom::Cpn::parse(&format!("virtual/choice_{id}")).unwrap(),
            None,
        );
        ...
        if allow_none {
            versions.push((Version::parse("0").unwrap(), vec![]));
        }
        ...
        for (i, child) in children.iter().enumerate() {
            ...
            versions.push((Version::parse(&ver_num.to_string()).unwrap(), deps));
        }
